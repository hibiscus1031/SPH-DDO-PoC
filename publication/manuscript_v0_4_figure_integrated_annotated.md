...
For component \(c\), signal resolvability compared a robust target scale \(T_c\) with qualified numerical and reference uncertainty \(U_c\) through \(R_c=T_c/U_c\). The ratio measures separation of defect magnitude from qualified numerical and reference uncertainty; it is not a physical signal-to-noise ratio or a measure of physical model uncertainty. The preregistered criterion required \(R_c\geq10\) and a stratified-bootstrap lower bound \(L_{95,c}>5\). Analytically unexcited component-case pairs were excluded from the component aggregation rather than inserted as zeros. This procedure tested whether a defect rose above the numerical uncertainty floor.
...
Observable identifiability was assessed by asking whether nearby particles in deployment-compatible feature space had sufficiently similar defects. In this manuscript, identifiability is an operational qualification concept based on the frozen empirical criteria, not a classical structural-identifiability proof. The diagnostics combined nearest-neighbour disagreement, conditional target variance, fixed non-neural oracle error and improvement, family robustness, and feature-space coverage. The disagreement median and 90th percentile could not exceed 0.25 and 0.60, respectively. Conditional-variance and oracle-error limits were 0.35 and 0.50, while coverage had to reach 0.90. Every applicable gate had to pass.
...
Fresh requalification used 384 newly generated complete cases, balanced as 96 cases per family. New deterministic phases and disorder realizations produced zero field-lineage overlap with the development atlas. Exactly 49,152 particle samples—the fixed 128-particle-per-case subsample—entered the formal H3 analysis. The separate directional-frame fallback audit covered all 627,264 particle environments in the fresh cases rather than only that formal subsample. The expanded representation and all diagnostic gates remained unchanged after fresh target access.
